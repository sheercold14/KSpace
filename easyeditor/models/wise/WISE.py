...
class WISE(torch.nn.Module):
    ...
    def _get_layer_obj(self):
        """Get the adapter layer object, compatible with bracket-indexed layer names."""
        return getattr(self.edit_module, self.layer_name)
    ...
    def reset_layer(self):
        ...
        if isinstance(layer, WISEAdapter):
            setattr(self.edit_module, self.layer_name, layer.original_layer)
        elif self.original_layer is not None:
            setattr(self.edit_module, self.layer_name, copy.deepcopy(self.original_layer))
    ...
    def generate(self, *args, **kwargs):
        setattr(self._get_layer_obj(), "key_id", -1)
    ...
    def edit(self, config, tokens, act_mask=None, deact_mask=None):
        ...
        setattr(self._get_layer_obj(), "training", True)
        setattr(self._get_layer_obj(), "editing", True)
        self.get_adapter_layer().set_parameter_tunable()
        if getattr(self._get_layer_obj(), "editing_total_cnt") % self.config.save_freq == 0:
        ...
        setattr(self._get_layer_obj(), "editing", False)
        setattr(self._get_layer_obj(), "training", False)

        editing_total_cnt = getattr(self._get_layer_obj(), "editing_total_cnt") + 1
        setattr(self._get_layer_obj(), "editing_total_cnt", editing_total_cnt)
        #
