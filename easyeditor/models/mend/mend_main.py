...
class MendMultimodalRewriteExecutor(MendRewriteExecutor):
    ...
    def apply_to_model(
        ...
        **kwargs
        ...
        weights_copy = {}
        model = deepcopy(self.model) if copy else self.model
        model_device = model._device() if hasattr(model, "_device") else next(model.parameters()).device
        ...
        image = torch.stack(image, dim=0).to(model_device)
        text_input = [s + t for s, t in zip(src, trg)]
        prompt_template = model.prompt_template if hasattr(model, "prompt_template") else "{}"
        
        wrapped_full = [prompt_template.format(text) for text in text_input]
        wrapped_prompt = [prompt_template.format(prompt) for prompt in src]
        encoded = tok(
            wrapped_full,
            return_tensors="pt",
            padding=True,
        ...
        prompts_len = []
        for idx, prompt_text in enumerate(wrapped_prompt):
            prompt_ids = tok.encode(prompt_text)
            prompts_len.append(len(prompt_ids))
            labels[idx, : min(len(prompt_ids), labels.shape[1])] = -100
