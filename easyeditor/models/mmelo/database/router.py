import os
import torch
from torchvision import transforms
import torch.nn.functional as F
from .db import *
from .tools import *

class Router:
    def __init__(self, config):
        self.config = config
        self.image_processor, self.image_encoder = self.init_image_encoder()
        self.text_encoder = self.init_text_encoder()
        self.VisionVecDB = VisionVecDB(config)
        self.VecDB = VecDB(config)
        self.memory_num = 0
        self.block_id = 0

    def _cache_dir(self):
        model_config = getattr(self.config, "model", None)
        cache_dir = getattr(model_config, "cache_dir", None) or getattr(self.config, "cache_dir", None)
        hub_dir = os.path.join(cache_dir, "huggingface", "hub") if cache_dir else None
        if hub_dir and os.path.isdir(hub_dir):
            return hub_dir
        return cache_dir

    def init_image_encoder(self):
        if self.config.image_encoder_name == 'dino':
            processor = transforms.Compose([
                    transforms.Resize(256),
                    transforms.CenterCrop(224),
                    transforms.ToTensor(),
                    transforms.Normalize([0.485, 0.456, 0.406],
                                        [0.229, 0.224, 0.225])
                ])
            encoder = torch.hub.load('facebookresearch/dinov2:main', 'dinov2_vitl14_lc', source='github', trust_repo=True)
            encoder = encoder.to(self.config.device)

        elif self.config.image_encoder_name == 'clip':
            from transformers import CLIPProcessor, CLIPModel
            clip_name = getattr(self.config, "clip_image_encoder_name", "openai/clip-vit-large-patch14-336")
            encoder = CLIPModel.from_pretrained(clip_name, cache_dir=self._cache_dir())
            encoder = encoder.to(f"cuda:{self.config.device}")
            processor = CLIPProcessor.from_pretrained(clip_name, cache_dir=self._cache_dir())
        elif self.config.image_encoder_name == 'vit':
            from transformers import ViTFeatureExtractor, AutoModel
            processor = ViTFeatureExtractor.from_pretrained('openai/clip-vit-base-patch32', cache_dir=self._cache_dir())
            encoder = AutoModel.from_pretrained('openai/clip-vit-base-patch32', cache_dir=self._cache_dir())
        else:
            raise ValueError("Unknown Encoder Name!")

        return processor, encoder

    def init_text_encoder(self):
        if getattr(self.config, "text_encoder_name", None) == "clip_text":
            if self.config.image_encoder_name != "clip":
                raise ValueError("clip_text requires image_encoder_name=clip")

            class CLIPTextEncoderAdapter:
                def __init__(self, processor, model, device):
                    self.processor = processor
                    self.model = model
                    self.device = device

                def encode(self, text):
                    inputs = self.processor(
                        text=[text],
                        return_tensors="pt",
                        padding=True,
                        truncation=True,
                    )
                    inputs = {key: value.to(self.device) for key, value in inputs.items()}
                    with torch.no_grad():
                        features = self.model.get_text_features(**inputs)
                    features = F.normalize(features, p=2, dim=1)
                    return features.squeeze(0).detach().cpu().numpy()

            return CLIPTextEncoderAdapter(
                self.image_processor,
                self.image_encoder,
                f"cuda:{self.config.device}",
            )

        try:
            from FlagEmbedding import FlagModel
            return FlagModel(
                self.config.text_encoder_name,
                query_instruction_for_retrieval='Represent this sentence for searching relevant passages:'
            )
        except ImportError:
            from sentence_transformers import SentenceTransformer

            class SentenceTransformerFlagAdapter:
                def __init__(self, model_name, cache_folder=None, device=None):
                    self.model = SentenceTransformer(
                        model_name,
                        cache_folder=cache_folder,
                        device=device,
                    )

                def encode(self, text):
                    return self.model.encode(
                        text,
                        convert_to_numpy=True,
                        normalize_embeddings=True,
                    )

            return SentenceTransformerFlagAdapter(
                self.config.text_encoder_name,
                cache_folder=self._cache_dir(),
                device=f"cuda:{self.config.device}",
            )

    def batch_embed(self, batch):
        if self.config.task=='vqa' :
            image, text_input = batch["ori_image"], batch["prompt"]
        else:
            image, text_input = batch["ori_image"], batch["prompt"]

        # Text Embedding of Query
        batch_query = [torch.from_numpy(self.text_encoder.encode(x)).unsqueeze(0).float() for x in text_input]
        batch_query = torch.cat(batch_query,dim=0)

        # Image Embedding of Query
        batch_query_vision = None
        # No Image for Text Locality
        if image is not None:
            if self.config.image_encoder_name == 'dino':
                image_preprocess = [self.image_processor(x).unsqueeze(0).to(f"cuda:{self.config.device}")
                                    for x in image]
                image_preprocess = torch.cat(image_preprocess,dim=0)
                batch_query_vision = self.image_encoder(image_preprocess)

            elif self.config.image_encoder_name == 'clip':
                image_preprocess = [self.image_processor(images= x, return_tensors="pt", padding=True)["pixel_values"]
                                    for x in image]
                image_preprocess  = torch.cat(image_preprocess,dim=0).to(f"cuda:{self.config.device}")
                batch_query_vision= F.normalize(self.image_encoder.get_image_features(image_preprocess),p=2,dim=1)

            elif self.config.image_encoder_name == 'vit':
                image_preprocess = [self.image_processor(images=x, return_tensors="pt", padding=True)["pixel_values"]
                                    for x in image]
                image_preprocess = torch.cat(image_preprocess, dim=0)
                batch_query_vision = F.normalize(self.image_encoder(pixel_values=image_preprocess).pooler_output, p=2, dim=1)
        return batch_query, batch_query_vision

    def database_batch_add(self, batch_query, batch_query_vision, idx):
        self.block_id = idx
        for idx, (query, query_vision) in enumerate(zip(batch_query, batch_query_vision)):
            self.VecDB.add_cluster(query.detach(), self.block_id, None, self.memory_num)
            self.VisionVecDB.add_cluster(query_vision.detach(), self.block_id, None, self.memory_num)
            self.memory_num += 1

    def search(self, batch_query, batch_query_vision):
        if batch_query_vision is None:
            text_dists = self.VecDB.search_combine_database_v2(batch_query)
            vision_dists = torch.zeros_like(text_dists)
        else:
            text_dists = self.VecDB.search_combine_database_v2(batch_query)
            vision_dists = self.VisionVecDB.search_combine_database_v2(batch_query_vision)
        lora_block_mapping = self.VecDB.search_vision_text_cluster(text_dists.to('cpu'),vision_dists.to('cpu'))
        return lora_block_mapping

    def get_lora_mapping(self, batch):
        batch_query, batch_query_vision = self.batch_embed(batch)
        return self.search(batch_query, batch_query_vision)
