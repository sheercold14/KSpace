...
class GRACE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        if type(original_layer) is not GRACEAdapter:
            setattr(self.edit_module, self.layer_name, GRACEAdapter(config, original_layer, transpose=transpose).to(self.device))
    ...
    def __call__(self, **kwargs):
        ...
        #     setattr(getattr(self.edit_module, self.layer_name), "key_id", key_id) # Tell GRACE which token to use for its query (default is the last token)
    ...
    def generate(self, *args, **kwargs):
        setattr(getattr(self.edit_module, self.layer_name), "key_id", -1)
    ...
    def edit(self, config, tokens, edit_id):
        ...
        setattr(getattr(self.edit_module, self.layer_name), "key_id", key_id)
        
        # --- pass edit label, training mode, and key_id into GRACE ---
        setattr(getattr(self.edit_module, self.layer_name), "training", True)
        setattr(getattr(self.edit_module, self.layer_name), "edit_label", tokens["labels"])
        setattr(getattr(self.edit_module, self.layer_name), "edit_id", edit_id)
                
        self.losses = []
        # --- train GRACE value ---
        for i in range(config.n_iter):
            # --- insert iteration into each layer (only initiate keys on iteration 1) ---
            setattr(getattr(self.edit_module, self.layer_name), "iter", i)
        ...
        setattr(getattr(self.edit_module, self.layer_name), "training", False)
        chosen_key = getattr(getattr(self.edit_module, self.layer_name), "chosen_key")
        nkeys = len(getattr(getattr(self.edit_module, self.layer_name), "keys"))
