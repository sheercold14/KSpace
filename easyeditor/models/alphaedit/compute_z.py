...
def compute_z(
    ...
    if native_vlm:
        rewriting_prompts = [
            request["prompt_template"].format(context.format(request["prompt"]))
            if request.get("prompt_template") else context.format(request["prompt"])
    ...
    if native_vlm:
        ...
        for i in range(len(all_prompts)):
            ...
            rewriting_targets[i, answer_start:seq_len] = probe_output.input_tokens[i, answer_start:seq_len]
            lookup_idxs.append(max(answer_start - 1, 0))
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            if native_vlm:
                logits = model(probe_sample).logits
