...
def exact_match_score(prediction, ground_truth):
    if isinstance(ground_truth, (list, tuple, set)):
        return any(exact_match_score(prediction, item) for item in ground_truth)
...
def _get_hparam_judge_model(hparams):
    return (
        getattr(hparams, 'judge_model', None)
        or getattr(hparams, 'api_model', None)
        or getattr(hparams, 'llm_judge_model', None)
    )


def _get_hparam_judge_base_url(hparams):
    return (
        getattr(hparams, 'judge_base_url', None)
        or getattr(hparams, 'api_base_url', None)
    )


def _get_hparam_judge_timeout(hparams):
    return getattr(hparams, 'judge_timeout', 60)


def _get_hparam_judge_max_retries(hparams):
    return getattr(hparams, 'judge_max_retries', 5)


def _get_hparam_judge_strict(hparams):
    return getattr(hparams, 'judge_strict', False)
...
Just return the letters "A" or "B", with no text around it.
    ...
    with open(api_key) as f:
        api_key_payload = json.load(f)
    model_name = model_name or api_key_payload.get('model') or api_key_payload.get('qwen_model') or "qwen-turbo"
    base_url = base_url or api_key_payload.get('base_url')
    if base_url is None:
        provider = str(api_key_payload.get('provider', '')).lower()
        api_key_file = os.path.basename(api_key).lower()
        base_url = (
            "https://dashscope.aliyuncs.com/compatible-mode/v1"
            if (
                api_key_payload.get('qwen_api_key')
                or provider in {"aliyun", "dashscope"}
                or "aliyun" in api_key_file
            )
            else "https://aihubmix.com/v1"
        )
    if "dashscope" in base_url or "aliyuncs" in base_url:
        api_key_data = (
            api_key_payload.get('qwen_api_key')
            or api_key_payload.get('qwen-turbo-key')
            or api_key_payload.get('key')
        )
    else:
        api_key_data = (
            api_key_payload.get('key')
            or api_key_payload.get('qwen-turbo-key')
            or api_key_payload.get('qwen_api_key')
        )
    if not api_key_data:
        raise KeyError(f"API key file must contain 'key', 'qwen-turbo-key', or 'qwen_api_key': {api_key}")
    ...
    if locality:
        ...
        if hasattr(hparams, 'api_key') and hparams.api_key:
            ...
            try:
                ...
                score = llm_judge_qwen(
                    edit_prompt['text_input'],
                    edit_prompt['answer'],
                    gen_content,
                    hparams.api_key,
                    model_name=_get_hparam_judge_model(hparams),
                    base_url=_get_hparam_judge_base_url(hparams),
                    timeout=_get_hparam_judge_timeout(hparams),
                    max_retries=_get_hparam_judge_max_retries(hparams),
                )
            except Exception as e:
                 if _get_hparam_judge_strict(hparams):
                     raise
